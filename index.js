...
function showBoardPopup(settings) {
    ...
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.6);z-index:999999;overflow-y:auto;display:block;';

    const popup = document.createElement('div');
    popup.classList.add('cb-popup');
    popup.style.cssText = 'position:relative;width:95%;max-width:480px;max-height:85vh;margin:5vh auto;border-radius:12px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.3);background:#ffffff;';
    ...
    closeBtn.addEventListener('touchend', (e) => { e.preventDefault(); closePopup(); });

    const contentArea = document.createElement('div');
    contentArea.classList.add('cb-popup-content');
    contentArea.style.cssText = 'overflow-y:auto;max-height:80vh;';
    ...
    overlay.addEventListener('touchend', (e) => {
        if (e.target === overlay) { e.preventDefault(); closePopup(); }
    });
    ...
    function showListView(page) {
        ...
        contentArea.querySelectorAll('.post-item[data-post-index]').forEach(el => {
            ...
            el.addEventListener('click', openPost);
            el.addEventListener('touchend', (e) => { e.preventDefault(); openPost(); });
        });
        ...
        contentArea.querySelectorAll('[data-page]').forEach(el => {
            ...
            el.addEventListener('click', goPage);
            el.addEventListener('touchend', (e) => { e.preventDefault(); goPage(); });
        });
    }
    ...
    function showDetailView(idx) {
        ...
        function goBack() { showListView(currentPage); }
...
function addMainButton() {
    ...
    button.addEventListener('touchend', openBoard);
